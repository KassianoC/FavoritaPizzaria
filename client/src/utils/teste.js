...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    bairro.price > 0 ? setTaxa(bairro.price) : setTaxa(0);
  }, [bairro, endereco]);

  useEffect(() => {
    if (bairro) {
      const taxaBairro = Object.values(bairros).filter((bairr) => bairr.name == bairro);
      setTaxa(taxaBairro[0].price);
    }
  }, [bairro]);
